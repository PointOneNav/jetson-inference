...
if [[ `uname` == 'Linux' && `uname -p` == "aarch64" ]]; then
	...
	bazel run -c opt //point_one/vision/maply/localization:orb -- --map ~/Downloads/map\ \(2\).bin --calibrationSFM ~/calibration/camera_calibration_flir_17391313.xml -pyramid 1  --alignment ~/Downloads/sim_unscaled.sim --origin ~/Downloads/origin\ \(2\).json
        ...
        sudo apt-get -y install git g++-5 gcc-5 default-jdk libxkbcommon-dev cmake python-pip python3-pip libboost-all-dev cmake

	### Copy gstreamer files ###
	sudo cp /usr/lib/x86_64-linux-gnu/gstreamer-1.0/include/gst/* /usr/include/gstreamer-1.0/gst/

	### Download appropriate nv-tensorrt library for OS ###
	cd $HOME
